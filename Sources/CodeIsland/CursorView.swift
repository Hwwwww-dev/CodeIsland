...
struct CursorView: View {
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // MOOD SCENES — idle variants for CursorBot (gem mascot)
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

    // HUNGRY — dual-freq wobble + open mouth + food emojis + belly dots
        ...
        ZStack {
            TimelineView(.periodic(from: .now, by: 0.05)) { ctx in
                ...
                Canvas { c, sz in
                    ...
                    drawLegs(c, v: v)
                    drawGem(c, v: v, dy: float + wobble)
                    // Open mouth — bright ellipse in face area
                    c.fill(Path(ellipseIn: v.r(4.8, 11.5, 3.4, 1.4, dy: float + wobble)),
                           with: .color(Self.lightC.opacity(0.88)))
                    // Rumble belly glow dots
                    let rumble: CGFloat = sin(t * 18) > 0 ? 0.5 : 0.0
                    for i: CGFloat in [0, 1, 2] {
                        let dotR = v.r(4.8 + i * 1.6, 13.0 + rumble * 0.2, 0.9, 0.9, dy: float + wobble)
                        c.fill(Path(ellipseIn: dotR),
                               with: .color(Color(red: 1.0, green: 0.6, blue: 0.15).opacity(0.55)))
                    }
                }
            }
            TimelineView(.periodic(from: .now, by: 0.05)) { ctx in
                ...
                ZStack {
                    ForEach(0..<5, id: \.self) { i in
                        ...
                        Text(foods[i % foods.count])
                            ...
                            .opacity(op)
                            .offset(x: size * CGFloat(-0.28 + ci * 0.15 + sin(p * .pi) * 0.12),
                                    y: -size * CGFloat(0.08 + p * 0.38))
                    }
                }
            }
        }
    ...
    // TIRED — slow nod + half-closed eyes (dimmed) + two large Zs
        ...
        ZStack {
            TimelineView(.periodic(from: .now, by: 0.06)) { ctx in
                ...
                Canvas { c, sz in
                    ...
                    drawLegs(c, v: v)
                    drawGem(c, v: v, dy: nod, shimmer: 0)
                    // Half-closed eye slits
                    c.fill(Path(v.r(3.8, 9.8, 1.5, 0.45, dy: nod)),
                           with: .color(Self.lightC.opacity(0.55)))
                    c.fill(Path(v.r(6.4, 9.8, 1.5, 0.45, dy: nod)),
                           with: .color(Self.lightC.opacity(0.55)))
                }
            }
    ...
    // SAD — dim gem body + blue tint + two tear streams + frown eyes
        ...
        ZStack {
            TimelineView(.periodic(from: .now, by: 0.06)) { ctx in
                ...
                Canvas { c, sz in
                    ...
                    var gctx = c
                    gctx.opacity = 0.70
                    drawGem(gctx, v: v, dy: float)
                    // Blue cold overlay
                    let gemR = v.r(2.5, 5.5, 10.0, 9.0, dy: float)
                    c.fill(Path(ellipseIn: gemR),
                           with: .color(Color(red: 0.3, green: 0.5, blue: 0.9).opacity(0.13)))
                    // Sad eyes — small half-ovals
                    c.fill(Path(v.r(3.8, 10.0, 1.5, 0.5, dy: float)),
                           with: .color(Self.lightC.opacity(0.70)))
                    c.fill(Path(v.r(6.4, 10.0, 1.5, 0.5, dy: float)),
                           with: .color(Self.lightC.opacity(0.70)))
                    // Frown mouth
                    c.fill(Path(v.r(4.5, 11.5, 0.8, 0.5, dy: float)),
                           with: .color(Self.lightC.opacity(0.50)))
                    c.fill(Path(v.r(5.3, 11.8, 1.8, 0.5, dy: float)),
                           with: .color(Self.lightC.opacity(0.50)))
                    c.fill(Path(v.r(7.1, 11.5, 0.8, 0.5, dy: float)),
                           with: .color(Self.lightC.opacity(0.50)))
                }
            }
            TimelineView(.periodic(from: .now, by: 0.05)) { ctx in
        ...
        ZStack {
            TimelineView(.periodic(from: .now, by: 0.05)) { ctx in
                ...
                Canvas { c, sz in
                    ...
                    drawGem(c, v: v, dy: float)
                    // Green sick overlay on gem body
                    let gemR = v.r(2.5, 5.5, 10.0, 9.0, dy: float)
                    c.fill(Path(ellipseIn: gemR),
                           with: .color(Color(red: 0.2, green: 0.78, blue: 0.3).opacity(0.17)))
                    // X eyes
                    let eyePos: [(CGFloat, CGFloat)] = [(3.8, 9.2), (6.4, 9.2)]
                    for (ex, ey) in eyePos {
                        c.fill(Path(v.r(ex, ey, 0.65, 0.65, dy: float)), with: .color(Self.lightC.opacity(0.9)))
                        c.fill(Path(v.r(ex + 0.65, ey + 0.65, 0.65, 0.65, dy: float)), with: .color(Self.lightC.opacity(0.9)))
                        c.fill(Path(v.r(ex + 0.65, ey, 0.65, 0.65, dy: float)), with: .color(Self.lightC.opacity(0.9)))
                        c.fill(Path(v.r(ex, ey + 0.65, 0.65, 0.65, dy: float)), with: .color(Self.lightC.opacity(0.9)))
                    }
                    c.translateBy(x: -shake * v.s, y: 0)
                }
            }
    ...
    // JOYFUL — bouncy hop + bright shimmer + 12-particle double ring
    private var joyfulScene: some View {
        ZStack {
            TimelineView(.periodic(from: .now, by: 0.05)) { ctx in
                ...
                let shimmer = sin(t * 2 * .pi / 0.8) * 0.5 + 0.5
                ...
                Canvas { c, sz in
                    ...
                    drawEyes(c, v: v, dy: jumpY, scale: blink)
                }
            }
